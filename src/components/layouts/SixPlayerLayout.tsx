...
interface SixPlayerLayoutProps {
  otherPlayers: any[];
  currentScoreCard: number | null;
}

export default function SixPlayerLayout({ 
  otherPlayers, 
  currentScoreCard 
}: SixPlayerLayoutProps) {
  return (
    <>
      {/* 上部のプレイヤー */}
      ...
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexGrow: 1 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'right', flex: 1 }}>
          <Card sx={{ flexGrow: 1, minWidth: "50%" }}>
            <Box sx={{ textAlign: 'center', py: 1 }}>
              <Typography variant="h6" component="div" gutterBottom>
                {otherPlayers[4]?.name || `プレイヤー${otherPlayers[4]?.id}`}
              </Typography>
              {otherPlayers[4]?.playedCard ? (
                <Paper 
                  elevation={3} 
                  sx={{ 
                    p: 3, 
